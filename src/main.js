...
function initGUI() {
	...
	gui
		.add(displayConfig, "VISCOSITY", 0, 30)
		.name("Viscous Diffusion Iterations");
	gui.add(displayConfig, "RADIUS", 2, 10).name("Radius/Brush Size");
	gui
		.add(displayConfig, "INPUT_MODE", [
			"Drag Fluid",
			"Draw Boundaries",
			"Erase Boundaries",
		])
	...
	gui
		...
		.name("Layer");
	gui
		.add(displayConfig, "VISCOUS_DIFFUSION_ON")
		.name("Enable Viscous Diffusion");
...
function initParticles() {
	...
	particleSim = new ParticleSim(
		grid_resolution,
		particleSpan,
		displayConfig.NUM_RENDER_STEPS,
		displayConfig.DELTA_TIME,
	);
	particleAge = new ParticleAge(
		grid_resolution,
		displayConfig.MAX_PARTICLE_AGE,
		particleSpan,
		displayConfig.DELTA_TIME,
	);
...
function initShaderLoaders() {
	...
	gridCellTex = new THREE.WebGLRenderTarget(
		grid_resolution.x,
		grid_resolution.y,
		{
			minFilter: THREE.LinearFilter,
	...
	/* This is for the actual render to the canvas. The canvasTex will be written to by boundaryRender, which takes in
       ...
       canvasTex. */
	canvasTex = new THREE.WebGLRenderTarget(
		grid_resolution.x,
		grid_resolution.y,
		{
			minFilter: THREE.LinearFilter,
...
function UpdateMousePosition(X, Y) {
	...
	externalVelocity.sourceDirection.x = Number.isFinite(velocityX)
		? velocityX
		: 0;
	externalVelocity.sourceDirection.y = Number.isFinite(velocityY)
		? velocityY
		: 0;
...
function runSimulation() {
	...
	if (!displayConfig.PAUSED) {
		...
		advector.advect_texture(
			renderer,
			velocityField.read_buf,
			velocityField.read_buf,
			0.999,
			1.0,
			velocityField.write_buf,
		);
		...
		if (displayConfig.INPUT_MODE === "Draw Boundaries") {
			arbitraryBoundary.draw_boundary(
				renderer,
				boundaryField.read_buf,
				displayConfig.RADIUS,
				1.0,
				boundaryField.write_buf,
			);
		...
		divergence2D.compute_divergence(
			renderer,
			velocityField.read_buf,
			divergenceField.write_buf,
		);
		...
		for (let i = 0; i < displayConfig.JACOBI_ITERATIONS; i++) {
			jacobi.compute(
				...
				-1.0,
				0.25,
				pressureField.read_buf,
				divergenceField.read_buf,
				pressureField.write_buf,
			);
		...
		for (let i = 0; i < displayConfig.NUM_RENDER_STEPS; i++) {
			...
			particleSim.renderToTarget(
				renderer,
				velocityField.read_buf,
				particleAgeState.read_buf,
				displayConfig.V_SCALE,
				particlePositions.write_buf,
			);
			...
			particleRender.renderToTarget(
				renderer,
				particlePositions.read_buf,
				velocityField.read_buf,
				particleTex,
			);
		}
